...
def normalizeImage(image):
    image=normalizeMinus1to1(image)
    return image
...
def maskimageMiddle(img):
    mask_height=img.shape[1]/3
    y1 = int(mask_height)
    y2 = int(mask_height*2)
    x1 =0
    x2 =img.shape[0]
...
def concatenateWithMiddle(original,predicted):
    partH=int(original.shape[1]/3)
    concatenated=original.copy()
    concatenated[partH:partH*2,0:original.shape[0],:]=predicted
    return concatenated
...
def denormalizeImage(img):
    #converts to pill image 0-255 data
    img=convert255(img)
